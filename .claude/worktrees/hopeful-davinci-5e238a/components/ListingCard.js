...
export default function ListingCard({
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/image?query=${encodeURIComponent(query)}&name=${encodeURIComponent(name)}&location=${encodeURIComponent(location)}&type=${encodeURIComponent(type)}&website=${encodeURIComponent(listing.website || '')}`
    )
      .then(r => r.json())
      .then(data => { if (!cancelled) setImgSrc(data.url || fallback); })
      .catch(() => { if (!cancelled) setImgSrc(fallback); });
    return () => { cancelled = true; };
  }, [listing, name, type, location, fallback]);
