...
function mockTryToDisplayAmount(tryAmount, currency) {
  const c = normalizeCurrency(currency);
  if (c === 'TRY') return tryAmount;
  if (c === 'EUR') return Math.max(40, Math.round(tryAmount / 37));
  if (c === 'USD') return Math.max(45, Math.round(tryAmount / 35));
  return tryAmount;
}

/** Mock uçuşlar — taban TRY; tercih edilen para birimine kabaca çevrilir (yaklaşık kur). */
export function getMockFlights({
  ...
  const flights = [];
  const cur = normalizeCurrency(preferredCurrency);
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const tryPrice = randomBetween(7000, 12000);
    flights.push({
      id: `mock-${i}-${al.code}`,
      airline: al.name,
      airlineCode: al.code,
      ...
      stops,
      direct: stops === 0,
      price: mockTryToDisplayAmount(tryPrice, cur),
      currency: cur,
