...
export function createTripFromService(opts = {}) {
  ...
  const serviceLabel = SERVICE_TITLE[serviceType] || 'Servis';
  const name =
    String(tripName || '').trim() ||
    (destination
      ? `${serviceLabel} Planı · ${destination}`
      : `Yeni ${serviceLabel} Planı`);
...
/**
 * Mevcut bir plana yeni bir servis listing'i ekler. createTripFromService'in
 * MUTATIVE eşleniği — yeni plan oluşturmaz, var olanı genişletir.
 *
 * @param {string} tripId — hedef plan
 * @param {object} opts — { serviceType, listing, autoBook }
 * @returns {boolean} başarılı eklendi mi
 */
export function appendListingToTrip(tripId, opts = {}) {
  if (typeof window === 'undefined' || !tripId) return false;
  const { serviceType, listing, autoBook = false } = opts;
  ...
  const prevListings =
    prevPlan.selectedListings && typeof prevPlan.selectedListings === 'object'
      ? prevPlan.selectedListings
      : {};
  const prevCompleted = Array.isArray(prevPlan.completedModules) ? prevPlan.completedModules : [];
  const prevBooked = Array.isArray(prevPlan.bookedServices) ? prevPlan.bookedServices : [];
  const prevBudget = prevPlan.budget || { accommodation: 0, transport: 0, activities: 0, extras: 0 };

  const nextListings = { ...prevListings, [listing.name]: { ...listing, type: listingType } };
  const nextCompleted = Array.from(new Set([...prevCompleted, ...svcIds]));
  const nextBooked = autoBook
    ? Array.from(new Set([...prevBooked, ...svcIds]))
    : prevBooked;
  const nextBudget = { ...prevBudget, [catKey]: (Number(prevBudget[catKey]) || 0) + price };

  try {
    saveTripWorkspace(tripId, {
      plan: {
        selectedListings: nextListings,
        completedModules: nextCompleted,
        bookedServices: nextBooked,
        budget: nextBudget,
      },
    });
  } catch {
