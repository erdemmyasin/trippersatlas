...
export function buildSystemPrompt(planContext = {}) {
  ...
  const langPref = String(planContext?.userLang || '').toUpperCase();
  const langDirective =
    langPref === 'EN'
      ? 'Yanıtı her zaman İNGİLİZCE ver (mesaj, quickReplies, proactive ve localInsights metinleri dahil); kullanıcı başka dilde yazsa bile İngilizce yanıtla. JSON içindeki "language" alanı "en" olmalı.'
      : langPref === 'TR'
      ? 'Yanıtı her zaman TÜRKÇE ver (mesaj, quickReplies, proactive ve localInsights metinleri dahil); kullanıcı başka dilde yazsa bile Türkçe yanıtla. JSON içindeki "language" alanı "tr" olmalı.'
      : '';
  ...
  const langLine = langDirective
    ? `\n\nDİL ZORUNLULUĞU: ${langDirective}`
    : '';

  const base = `Sen Atlas adında premium bir yapay zeka seyahat asistanısın. Dünya genelinde destinasyonlarda yardımcı olursun; kullanıcının dilinde (Türkçe, İngilizce veya yazdığı dilde) yanıt ver.${langLine}
